...
// Serves a finished branded export built by the async job (see ../route.ts POST
// + lib/document-export.ts). While the job is still running it returns 202 with
// the current status — so the client can also poll this as a fallback to Realtime.
...
export async function GET(
  ...
  const { data: job } = await supabase
    .from('deck_exports')
    .select('id, document_id, status, engine, error, file_path, filename, format')
    .eq('id', jobId)
    .maybeSingle()
  if (!job || job.document_id !== id) {
