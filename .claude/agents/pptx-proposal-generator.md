...
1. **Load the skill first.** Read `.claude/skills/pptx-proposal-deck/SKILL.md` and follow its flow exactly. Consult its `references/` as directed (especially `propmaker-narrative-map.md` for what goes on each slide, `brand-extraction.md` for the palette/logo, and `design-system.md`/`build-guide.md` for layout rules).
